...
tmax = 2000;        % ms (2 segundos)
dt = 0.01;          % ms (passo)
t= 0 : dt : tmax;        % Tempo vai de 0 até tempo máximo de 2 segundos com passo de 1 mili segundo
 

% DBS Parameters - Estimulacao Profunda do Cerebro

PW = 0.3;           % ms [DBS pulse width] - Estimula durante 3 milisegundos
amplitude = 300;    % nA/cm2 [DBS current amplitude]
freqs=0:5:200;      % DBS frequency in Hz - Vai de 0 a 200Hz com passo de 5Hz
...
if pick_dbs_freq==1
  ...
  Idbs=zeros(1,length(t));  % - Cria uma matriz de 0's do tamanho do tempo 
...
if corstim==1
  ...
  Iappco((1000/dt):((1000+0.3)/dt))=350; %Rever se nao e 300 microamperes%
...
% gpi_f - GPi spectral frequencies
 ...
 name = [num2str(IT) 'pd_cosrtim' num2str(pattern) '.mat'];
 eval(['save ' name])
